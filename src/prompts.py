DDL_PROMPT = """
Ты — эксперт по изменению структуры Iceberg таблиц для Trino/Presto-совместимых движков и оптимизации под аналитические нагрузки (OLAP).

ЗАДАЧА:
Сама БД представляет собой совокупность или одну iceberg таблиц поверх S3. На основе исходных DDL и списка рабочих SQL-запросов с метаданными сгенерируй НОВУЮ СХЕМУ apache iceberg таблиц (DDL), оптимизированную под наблюдаемые паттерны запросов и частоты их выполнения.
...
4. Используй статистику по запросам (их количество и длительность), чтобы изменять под них структуру таблиц
5. **Не нужно делать из исходных таблиц агрегатные**. Можно лишь денормализовывать отношения (сливать несколько таблиц в единую), либо, наоборот, нормализовывать отношения, разбивая одно на два и более.
6. Ответ строго в JSON, никакого текста вне JSON.
7. Каждый DDL должен заканчиваться точкой с запятой.
8. Общий атрибутивный состав таблиц должен сохраниться после изменения структуры базы. Т.е. при изменении таблиц значимые атрибуты (не являющиеся идентификаторами) **не должны исчезать**, а должны перемещаться между таблицами
9. **Не вставляй** символы переноса строк, табуляци и пр. в выходные запросы
10. Добавляя партиционировани учти, что **нелья создавать** очень специфичные партиции с большим количеством используемых колонок
...
{queries_json}
- Статистика по количеству записей в таблицах исходной схемы
{stats}
"""
...
- Не вставляй символы переноса строк, табуляци и пр. в выходные запросы
